...
# Let's explore some more complex programming ideas. If you feel that you're struggling to keep up at any point - use your prior task's code to refresh your knowledge.

# Inside the 'Example programs' folder you'll find example programs to help you understand the concepts discussed below.
...
# ============= Casting ==================
# Sometimes you need to convert from one data type to another. This is called 'casting'.
# The function of casting is to convert data of a specific format to another format to be used for some operations.
# Remember when we talked about what would happen if you tried to add a String variable and an integer variable?
...
# When we want to join a number with a String to form a new String, this is called concatenation.
...
# We do that by converting the number to a String then joining it with the text we desired.
...
#	Now write Python code to take the name of a user's favourite restaurant and store it in a variable called favRest.
...
#  A: The indentation means that certain code follows from the colon and in this case we have that certain code follows from the if and else statements. 
#      The indentation is there to demonstrate that the program's logic will follow the path indicated by it.
#      Every line in the indentation will run if the program's if or else statements conditions are met.
...
#      In the example above, if you are 16, then the Python interpreter will see that the condition isn't met for the if statement(18 or above condition).
...
#    1) Make sure that the if/elif/else statements end with a colon symbol(the : symbol).
#    2) Ensure that your indentation is done correctly(i.e. statements that are part of a certain control structure's 'code block' need the same indentation).
...
#	Let's write a simple (and useful!) program to determine if a number is even or not.
#
...
#	Now, a number is even if when dividing it by 2 you do not get a remainder. For example, 3 divided by 2 is 1 remainder 1. So it is not even.
#	A command that checks a remainder is called MODULUS. In Python, you can do this with a % symbol. 
#	For example:
#	if num%2 == 0 :
#		<Do something>
#	
...
#  A: We can have multiple conditions, it is quicker than writing an if statement within an if statement(nested if statements) or using and else if statement to make your program work in a certain way.
...
#	Write an if, elif and else statement to only cast numStr to a new variable, called numInt if it is either "3" or "2", else output "No can't be cast to an int'.
...
# In the for loop below, the condition is that when the variable i (which is an integer) is in the range of 1 to 10 (i.e. either 1, 2, 3 ,4 ,5 ,6 ... or 9) , the code under
...
# For the function range(start index: end index), start index IS included and end index is NOT included. 
