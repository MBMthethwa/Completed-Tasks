...
# Hopefully, this has so far been a recap of programming skills you already know.
...
# The while loop will continue to execute until its condition is no longer true.
...
#	Then, create a while loop that encodes that while 'start' is not even (i.e. divisible by 2 with no remainder), it will print out the value of 'start', and then add 1 to it.
...
#	Under the while loop, type a print statement to output "You guessed correctly!".
...
#	   Enter an item: Dog               -> Start of for loop asking for items to put into the list.
#	   Enter an item: Cat               -> 2nd run of the for loop.
#	   Enter an item: Harry             -> 3rd and last run of the for loop.
...
# ============= Comparing Strings and if-elif-else statements ==================  
...
if name == "Mary": # First check.
	...
	print "Parker"
else:                 # If all the checks above fail.
	print "Ok :(" 
	
# You can of course only have an if statement or an if statement with as many elif's as you want or just an if-else. You cannot just have an elif or else statement.


# ============= The difference between assigning things to variables and checking if a variable has a certain value ==================  	
# Note that name = "John" means that you are assigning the string John to the name variable(assignment statement). 
...
#	The program should take an integer as input from the user, and generate a random integer from 1 to 10 using the command random.randint(1,10).
#	Only using one if condition, have the program output "True" when the users integer is bigger than the random integer. Once that is working get the program to print 'True' if the number is bigger and the integer is also even.
#
# ==================================================================================================
...
#	For example, if the user enters 3, the program should ask them three separate times for a String.
...
#	The program should take in a user's name.  Only if his name is "Riaz" OR "Tom" will it execute a for loop asking 
#	for the user to add items to a list (similar to the above optional exercise). 
...
# === Make sure you have read and understand all of this example2.py file.
