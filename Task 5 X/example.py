...
# You will need to use knowledge from previous Tasks as these are the fundamentals of programming and techniques found in these Tasks apply to almost every programming language -> So dont be afraid to refer back!
...
print word[0:] # Prints 'Hello' - i.e. all the characters from the 0th position (the start of the String) to the end.
 
# A String can be indexed EXACTLY like a Python list because a String is basically a list of characters! 
 
print word [1:2] # Useful for String manipulation. Remember you can even index variables.
index = 2
...
# For example, if you wrote several lines of code to define logic that given a file name, opens that file, reads the contents, and prints it out to the screen, it may be useful to 'save' that code somewhere so you could easily reuse it.
# A programmer would define a function, named something like 'read_file', that would encode this logic.
# That way, the next time they need to read the contents of a file they simply 'call'(use) the function 'read_file' which will 'return' the result of that function, which in this case is the output to be printed to the screen.
...
# In programming, read_file(fileName) is a function that when called with a value stored in the variable fileName, returns some value or achieves some computation.
# Without knowing it, you've already used functions. The 'write' command used below, i.e. ofile.write(name+"\n") is in fact a function implemented by some programmer that tells Python how to write the content you give it to the file ofile.
...
num = 10
numPlusOne = addone(num) # The result that the 'addone' function 'returns' is stored in the numPlusOne variable. 
...
# Think of that 'call to the function', i.e. addone(num) , as a 'placeholder' for some computation. The function will go off and run its code, and return its result in that place.
...
# For example, though we have defined the function 'addone' above. The code indented underneath it would never be executed unless there was another line that 'called' addone with the command addone(some_variable) SOMEWHERE IN THE MAIN BODY OF YOUR CODE.
...
# Values passed in to the function through the 'function's parameters' (variables e.g. num1 and num2 above) will be referenced with those variable names. Think of it as copying in the values from the inputted parameter values when 'calling' the function to these 'function parameters'.
...
# y will store the special value None. This is not a String or any other data type, so it will cause an error when trying to do other things with it.
...
# The math library has many, many methods/functions. To find out more about them, you'd have to search something like 'math module Python' in Google to find out the various different function calls, what inputs they take, and what they output.
...
# If you want to square a number, the math module has a function: 'pow', that takes two numbers as input - the first is the number you want to raise to a power, and the second is the power number.
# Squaring a number is the same as raising that number to the power of two.

square = math.pow(5,2) # The pow function will take the first argument (5) and put the second argument (2) as an exponent for the first number. So we get 5^2 here which is 25.
...
#   Inside it, import the Python math module right at the top.
#   Now define a simple function inside squareme.py called square, that takes in a parameter(variable) called 'number'.
#   How to do this?  Remember to define a function like 'def functionname(nameOfVariableToTakeIn).
#   Then indented under that what you want to do with the variable in the function.
#   Take the variable and apply the math.pow(<number1>, <number2>) method to it.
...
# Files are an important source of information in Python. Before moving on, we want to make sure you know how to read and write to the most simple type of files - text files - using Python.
# Python includes a built-in file type. This is a bit like a String data structure, but much more complex. 

# The line below creates a new file 'object'(type) named f that is linked to the example.txt file in this folder. You'll learn about objects in a later task.

f = file('example.txt', 'r+')

# This means f is open for reading. The first argument (example.txt) is the filename and the second argument is the
...
# Here we intend to read and write from/to example.txt, which is already in the same folder as this file, example.py. Python will look in this directory for 'example.txt', and try read its context.
...
f.close() # Always close files when done with them, using this function call in order to free up the resources it was using. Notice this is a function that takes in 0 input.
...
#	There is already a text file in this document called 'example.txt'. 
...
# 	The text file does not print out, rather you get a text output of something in <   >.
...
#   When you see the output of 'print reader.readlines()' you may notice that each line is displayed in a LIST in the output (i.e. it has [   ] around it).
#   This is because Python automatically stores each line of the text file in a list for you.
#   Try adding 'print reader.readlines()[0]' to your code and now check the output. 
#   Do you understand what's happening? reader.readlines() returns a list. We automatically index the list to the first element in the list by adding [0] after readlines(). Then we print it out!
#   It's the same as working with lists in the earlier task. When we had list = ['A','B']  print list[0] outputted the first item of the list - 'A'!
#
# ==================================================================================================
...
name = raw_input("Enter your name: ") # We ask the user for their name. When they enter it, it is stored as a String in the variable name.

ofile.write(name+"\n") # We use the write function to write the contents of the variable name to the text file, which is represented by the object (a special type of variable you will learn more about later) ofile.
...
#   Remember how in a previous task you wrote a program that takes in names while the user hasn't entered 'John'?
...
#   Copy and paste your code from John.py in Task 1 into this folder into a new Python file called JohnWrite.py. That previous task folder may have been moved to the 'Completed Work' folder.
#   At the end of your code, write the single line to create a new text file in this folder called 'IncorrectName.txt'. Don't know how to do this? Look at the section above -> we just told you how which python code creates a new file 'output.txt' in write mode :P
#   Now loop through the list of incorrect names using a for loop, i.e. 'for name in names' if your list was saved in a variable called names.
...
# You should define a separate function in your code, called countDifference, that takes in a String and returns an int. 
...
# But maybe it's easier to write new logic for this than have to go and find out which module has such a function defined? Who knows!
