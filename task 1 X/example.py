...
# Code can be written in different programming LANGUAGES - Python, Java, C are some examples of programming languages.
...
# We write commands, or code in the programming language Python and save these commands in a Python file. A Python file has the following file naming format: filename.py (where filename can be any valid file name and .py is the file extension in Windows).
# You can then 'run' the Python file. The python program you have written is executed and display outcomes may result based on what the code statements say.
...
# Almost all programming languages share common SYNTAX, or share ways of writing statements.
...
#  A: Give your variable a name, and give it some information as seen below.
...
#  Q: If I can give a variable information, can I change the information that the variable holds?
#  A: Yes, after we set num to be 2, the subsequent line above changes the variable num and assigns it a value of 3.

#  Q: So can I use a sentence as a variable name?
#  A: No, there are naming restrictions on variables so you have to comply. Note that Python has certain keywords and reserved words that cannot be used to name variables. 
...
# 	We write Python code statements in text editors like Notepad++ or the IDLE Python GUI so that all our variable definitions and code are saved. We can then run these files as Python programs at any time we want, and we can use these 
...
# ========= Data types ===========
#  Q: Do variables store specific kinds of values? 
...
#      Integers are detected with the lack of quotation marks and presence of digits or another accepted number format.
#      Floats are detected with the presence of decimal point numbers.
#      Take heed that types can be converted from one to another, and you need to take care when setting a string with numerical information for example and your intention thereof.
...
#  A: Yes, remember that the number will be read by Python from the variable. Similarly in mathematics, variables which are given numbers are interchangeable with their values.
#      One can simply plug in the variable's value in an arithmetic expression. 
...
# e.g. You can add two variables if they are both integers and the result will also be an integer as expected.
...
# The variable fullName now stores the string "Phil Jones" , we have concatenated the two strings.
...
# 
# cd Dropbox
# cd <this folder name -> should be your firstname and first initial of surname e.g. RiazM 
# cd Task 1
# python example.py
